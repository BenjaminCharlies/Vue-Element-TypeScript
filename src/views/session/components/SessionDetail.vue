      ...
      <div class="box-card">
        <el-form ref="sessionForm" :model="sessionForm" :rules="sessionRules" class="session-form" label-width="300px">
          ...
          <el-form-item label="Exclusive Session">
            ...
            <el-col v-show="sessionForm.isExclusiveSession" :span="4" style="margin-left:10px">
              <el-input v-model="sessionForm.pin" size="mini" :disabled="true" />
            </el-col>
          </el-form-item>
          <el-collapse-transition>
            <el-form-item v-if="sessionForm.sessionLateEntryCharges && sessionForm.sessionLateEntryCharges.length > 0" label="late Stage">
              <el-form ref="sessionForm.sessionLateEntryCharges" :model="sessionForm.sessionLateEntryCharges">
                <el-table border :data="sessionForm.sessionLateEntryCharges">
                  <el-table-column label="Stage" prop="stage">
                    <template slot-scope="scope">
                      Stage {{ scope.row.stage }}
                    </template>
                  </el-table-column>
                  <el-table-column label="Start Time" prop="startDate">
                    <template slot-scope="scope">
                      {{ scope.row.startDate | moment('YYYY-MM-DD') }}
                    </template>
                  </el-table-column>
                  <el-table-column label="End Time" prop="endDate">
                    <template slot-scope="scope">
                      <el-date-picker
                        v-if="scope.$index !== sessionForm.sessionLateEntryCharges.length - 1"
                        v-model="scope.row.endDate" size="mini"
                        style="width:130px"
                        @change="handleLateStageTimeChange(scope.row.endDate, scope.$index)"
                      />
                      <span v-else>{{ scope.row.endDate | moment('YYYY-MM-DD') }}</span>
                    </template>
                  </el-table-column>
